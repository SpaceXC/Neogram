...
fun normalizeCodePoint(codePoint: Int) =
    Character.toLowerCase(codePoint) // TODO: add an option to remove diacritics when they will be properly supported by server
...
@JvmOverloads
fun parseFloat(str: String?, defaultValue: Float = 0.0f): Float =
...
@JvmOverloads
fun indexOf(source: CharSequence, target: String, startIndex: Int = 0): Int =
    source.indexOf(target, startIndex)

@JvmOverloads
fun indexOfAny(source: CharSequence, target: CharArray, startIndex: Int = 0): Int =
    source.indexOfAny(target, startIndex)

fun length(str: CharSequence?): Int = str?.length ?: 0

fun String?.limit(limit: Int): String? =
    if (this.isNullOrEmpty() || this.length <= limit) this else this.substring(0, limit)
...
fun join(separator: String, separatorForLastItem: String, list: List<String>): String {
    ...
    var totalSize: Int =
        0.coerceAtLeast(separator.length * (size - 2)) + separatorForLastItem.length
    var index = 0
    for (item in list) {
