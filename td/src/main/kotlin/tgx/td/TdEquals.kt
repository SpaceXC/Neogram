...
internal fun <T> Array<T>?.contentEqualsOrEmpty(
    other: Array<T>?,
    contentEqual: T.(T) -> Boolean
): Boolean {
    return when {
...
@JvmOverloads
fun Array<PollOption>?.equalsTo(
    other: Array<PollOption>?,
    onlyTextContent: Boolean = false
): Boolean = this.contentEqualsOrEmpty(other) { b ->
    this.equalsTo(b, onlyTextContent)
}

@JvmOverloads
fun Array<TextEntity>?.equalsTo(
    other: Array<TextEntity>?,
    ignoreDefaultEntities: Boolean = false
): Boolean = when {
    this === other -> true
...
fun DraftMessage?.equalsTo(b: DraftMessage?, ignoreDate: Boolean = false): Boolean {
    return when {
        ...
        this == null || b == null -> false
            ...
            (this.date == b.date || ignoreDate) && this.replyTo.equalsTo(b.replyTo) && this.inputMessageText.equalsTo(
                b.inputMessageText
            )
        }
    }
