...
fun MessageSelfDestructType?.selfDestructsImmediately(): Boolean =
    this?.constructor == MessageSelfDestructTypeImmediately.CONSTRUCTOR
...
fun SearchMessagesFilter.isDocumentFilter(): Boolean =
    this.constructor == SearchMessagesFilterDocument.CONSTRUCTOR

fun SearchMessagesFilter.isPinnedFilter(): Boolean =
    this.constructor == SearchMessagesFilterPinned.CONSTRUCTOR

fun SearchMessagesFilter.isEmptyFilter(): Boolean =
    this.constructor == SearchMessagesFilterEmpty.CONSTRUCTOR
...
fun Message?.matchesFilter(filter: SearchMessagesFilter?): Boolean {
    ...
    return when (filter.constructor) {
        ...
        SearchMessagesFilterUrl.CONSTRUCTOR -> {
            (this.content.constructor == MessageText.CONSTRUCTOR && (this.content as MessageText).linkPreview != null) || this.content.textOrCaption()
                .hasLinks()
        }
...
fun PageBlock.findReference(name: String): RichText? {
    return when (this.constructor) {
        ...
        PageBlockSlideshow.CONSTRUCTOR -> {
            ...
            slideshow.caption.text.findReference(name) ?: slideshow.caption.credit.findReference(
                name
            )
        }
...
fun Usernames?.secondaryUsernamesCount(): Int {
    return if (this != null) {
        if (this.editableUsername.isNullOrEmpty()) {
            ...
            val disabledUsernamesCount =
                this.disabledUsernames.count { it != this.editableUsername }
...
fun Usernames?.findUsername(username: String, allowDisabled: Boolean = false): Boolean {
    if (username.isNotEmpty() && this != null) {
        ...
        if (this.editableUsername.isNotEmpty() && this.editableUsername.equals(
                username,
                ignoreCase = true
            )
        ) {
...
@JvmOverloads
fun Supergroup?.findUsername(username: String, allowDisabled: Boolean = false): Boolean =
    this?.usernames.findUsername(username, allowDisabled)

@JvmOverloads
fun User?.findUsername(username: String, allowDisabled: Boolean = false): Boolean =
    this?.usernames.findUsername(username, allowDisabled)

@JvmOverloads
fun Usernames?.findUsernameByPrefix(
    ...
    if (usernamePrefix.isNotEmpty() && this != null) {
        ...
        if (this.editableUsername.isNotEmpty() && this.editableUsername.startsWith(
                usernamePrefix,
                ignoreCase = true
            )
        ) {
...
@JvmOverloads
fun User?.findUsernameByPrefix(usernamePrefix: String, allowDisabled: Boolean = false): Boolean =
    this?.usernames.findUsernameByPrefix(usernamePrefix, allowDisabled)

@JvmOverloads
fun Supergroup?.findUsernameByPrefix(
    usernamePrefix: String,
    allowDisabled: Boolean = false
): Boolean = this?.usernames.findUsernameByPrefix(usernamePrefix, allowDisabled)
...
fun buildOutline(
    ...
    sticker: Sticker?,
    targetWidth: Float,
    targetHeight: Float = targetWidth,
    out: Path? = null
): Path? {
...
@JvmOverloads
fun buildOutline(
    contours: Array<ClosedVectorPath>?,
    sourceWidth: Int,
    sourceHeight: Int,
    targetWidth: Float,
    targetHeight: Float,
...
fun MessageReplyInfo?.hasUnread(lastGlobalReadInboxMessageId: Long = 0): Boolean {
    return this != null && this.lastMessageId != 0L && this.lastMessageId > max(
        lastGlobalReadInboxMessageId,
...
fun ChatInviteLink.isTemporary(): Boolean =
    this.expirationDate != 0 || this.memberCount != 0 || this.memberLimit != 0
...
fun PushMessageContent.isSticker(): Boolean =
    this.constructor == PushMessageContentSticker.CONSTRUCTOR
...
fun newSendOptions(
    ...
    updatedSchedulingState: MessageSchedulingState? = null
): MessageSendOptions {
...
fun newSendOptions(
    options: MessageSendOptions?,
    updatedSchedulingState: MessageSchedulingState?
): MessageSendOptions =
    newSendOptions(
        options = options,
...
fun newSendOptions(
    forceDisableNotification: Boolean,
...
@JvmOverloads
fun JsonValue.intValue(defaultValue: Int = 0): Int =
    this.numberValue(defaultValue.toDouble()).roundToInt()

@JvmOverloads
fun JsonValue.longValue(defaultValue: Long = 0): Long =
    this.numberValue(defaultValue.toDouble()).roundToLong()
...
fun TextEntityType.isEmailAddress(): Boolean =
    this.constructor == TextEntityTypeEmailAddress.CONSTRUCTOR

fun TextEntityType.isBankCardNumber(): Boolean =
    this.constructor == TextEntityTypeBankCardNumber.CONSTRUCTOR

fun TextEntityType.isBotCommand(): Boolean =
    this.constructor == TextEntityTypeBotCommand.CONSTRUCTOR
...
fun TextEntityType.isMentionName(): Boolean =
    this.constructor == TextEntityTypeMentionName.CONSTRUCTOR

fun TextEntityType.isUnderline(): Boolean = this.constructor == TextEntityTypeUnderline.CONSTRUCTOR
fun TextEntityType.isStrikethrough(): Boolean =
    this.constructor == TextEntityTypeStrikethrough.CONSTRUCTOR
...
fun TextEntityType.isCustomEmoji(): Boolean =
    this.constructor == TextEntityTypeCustomEmoji.CONSTRUCTOR
...
fun InputMessageContent.isVoiceNote(): Boolean =
    this.constructor == InputMessageVoiceNote.CONSTRUCTOR

fun InputMessageContent.isVideoNote(): Boolean =
    this.constructor == InputMessageVideoNote.CONSTRUCTOR
...
fun MessageContent.isProximityAlertTriggered(): Boolean =
    this.constructor == MessageProximityAlertTriggered.CONSTRUCTOR
