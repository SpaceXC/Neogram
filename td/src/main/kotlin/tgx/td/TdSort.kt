...
fun Array<Session>.sort() {
    this.sortWith(compareByDescending<Session> { it.isCurrent }.thenByDescending { it.isPasswordPending }
        .thenByDescending { it.lastActiveDate })
}

fun Array<LanguagePackInfo>.sort(activeLanguagePackId: String) {
    this.sortWith(compareByDescending<LanguagePackInfo> { it.isInstalled() }.thenBy { it.isBeta }
        .thenByDescending { it.isOfficial }
        .thenByDescending { !it.isInstalled() && activeLanguagePackId == it.id })
}

fun MutableList<Message>?.sortByViewCount() = this?.filter { it.interactionInfo != null }
    ?.sortedWith(compareByDescending<Message> { it.interactionInfo!!.viewCount }.thenByDescending { it.interactionInfo!!.forwardCount })